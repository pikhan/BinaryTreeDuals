...
% Jean-Yves Tinevez <tinevez@pasteur.fr> March 2012
    ...
    methods
        ...
        function [obj, root_ID] = tree(content, val)
            ...
            %
            % t = TREE(parent_pointer) initializes a new tree
            % from a parent pointer
            ...
            elseif isa(content, 'double')
                % parent pointer constructor
                obj.Parent = transpose(content);
                obj.Node = transpose(cellstr(string(1:length(content))));  
                root_ID = find(content == 0);
            else
